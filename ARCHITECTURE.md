...
jettoptx-aaron-public (Cloudflare Worker)
  ...
  ├── GET /.well-known/agenc-store.json (public unsigned AgenC store JSON, never 402)
...
- Public AgenC store: `GET /.well-known/agenc-store.json` is unauthenticated 200 JSON on `mcp.jettoptics.ai` / `aaron.jettoptics.ai` only (never 402, never `payTo` header, unsigned, `listings: []`). Not apex `jettoptics.ai`. Does not overwrite `/.well-known/agent-card.json`. Worker never signs.
