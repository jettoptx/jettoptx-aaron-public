...
OAuth Approve authenticates a hidden `csrf_token` HMAC-JWT (`typ=oauth-csrf`, signed with `MCP_OAUTH_SIGNING_KEY` or `MCP_API_KEY` — no new required secret). Payload is TTL (`exp`) + `client_id` + `redirect_uri` + `state` + nonce. Grok's iPhone embedded webview is third-party and will not return cookies even with `SameSite=None; Secure`; cookie-absent POST + valid signed form succeeds. Forged / expired / mismatched `client_id`|`redirect_uri`|`state` / cookie-only POSTs fail. A best-effort `JOE_OAUTH_CSRF` cookie is still set as `Path=/oauth; HttpOnly; SameSite=None; Secure` on HTTPS (`Domain=mcp.jettoptics.ai` host-only, never `.jettoptics.ai`). GET expires leftover `Path=/` cookies. HTTP localhost stays `SameSite=Lax`. Successful Approve returns **HTML 200** (meta refresh + `window.top.location` / `location.replace` + a visible Continue link) instead of a bare 302 — Grok's webview swallows cross-site 302 after form POST. Authentik / Google subscriber CSRF is origin-side (AstroJOE / Jetson `:8811`) — not implemented here.
